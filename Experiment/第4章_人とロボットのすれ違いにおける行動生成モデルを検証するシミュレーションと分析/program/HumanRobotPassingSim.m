...
% If use another condition, following conditions are considered.

% if Sim == 0
%     CompassionCondition = 'NoCompforStraight';
% elseif Sim == 1
%     CompassionCondition = 'CorrectCompforStraight';
% elseif Sim == 2
%     CompassionCondition = 'IncorrectCompforStraight';
% elseif Sim == 3
%     CompassionCondition = 'NoCompforBelow';
% elseif Sim == 4
%     CompassionCondition = 'CorrectCompforBelow';
% elseif Sim == 5
%     CompassionCondition = 'IncorrectCompforBelow';
% elseif Sim == 6
%     CompassionCondition = 'NoCompforAbove';
% elseif Sim == 7
%     CompassionCondition = 'CorrectCompforAbove';
% elseif Sim == 8
%     CompassionCondition = 'IncorrectCompforAbove';
% end


trialN = 1;   % Trial time in all simulation.
...
% o2: Compassion for Human preference
%-------------------------------------------------------------------------

% Nothing.
% If consider robot compassion condition, following cases.

%{
COMP_MAP_HinR = zeros(MAPw,MAPl);       % Mapping matrix indicating utility of prefered location.
CLh = 1/MAPl;                           % Most distant location indicate CLh*MAPl.

for i = 1:MAPl      % Prefered location about horizontal line
    COMP_MAP_HinR(:,i) = CLh*(MAPl-i+1);
end

if Sim == 4
    for i = 1:MAPw      % Prefered location about horizontal line
        COMP_MAP_HinR(i,:) = COMP_MAP_HinR(i,:)*(1/MAPw)*i;
    end
elseif Sim == 5
    for i = 1:MAPw      % prefered location about horizontal line
        COMP_MAP_HinR(i,:) = COMP_MAP_HinR(i,:)*(1/MAPw)*i;
...
elseif Sim == 7
    for i = 1:MAPw      % prefered location about horizontal line
        COMP_MAP_HinR(i,:) = COMP_MAP_HinR(i,:)*(1/MAPw)*(MAPw-i+1);
    end
elseif Sim == 8
    for i = 1:MAPw      % prefered location about horizontal line
        COMP_MAP_HinR(i,:) = COMP_MAP_HinR(i,:)*(1/MAPw)*(MAPw-i+1);
...
for j = 1:No(2)     % Encoding preference at end time point.
    [n,m] = ind2sub(size(COMP_MAP_HinR),j);
    Crcc{2}(j,1) = COMP_MAP_HinR(n,m);     % At this time, only encoded in at first time point.
end

CTh = 1;      % Human wants to arrive at distant location in early time point(>1)

for k = 1:t     % Encoding preference at all time point
    Crcc{2}(:,k) = Crcc{2}(:,1)/k;
end

% If consider incorrect compassion, reverse value relationship.
if Sim == 2
    COMPi_MAP_HinR = fliplr(COMP_MAP_HinR);
elseif Sim == 5
    COMPi_MAP_HinR = flip(COMP_MAP_HinR);
elseif Sim == 8
    COMPi_MAP_HinR = flip(COMP_MAP_HinR);
else
    COMPi_MAP_HinR = COMP_MAP_HinR;
end
...
% Thus, compassion for human preference completely encoded(Cr{2}).
...
% If consider another human preference condition, following case.
%{

if Sim == 3
    for i = 1:MAPw      % Prefered location about horizontal line
        PREFER_MAP_H(i,:) = PREFER_MAP_H(i,:)*(1/MAPw)*i;
    end
elseif Sim == 4
    for i = 1:MAPw      % Prefered location about horizontal line
        PREFER_MAP_H(i,:) = PREFER_MAP_H(i,:)*(1/MAPw)*i;
...
elseif Sim == 6
    for i = 1:MAPw      % Prefered location about horizontal line
        PREFER_MAP_H(i,:) = PREFER_MAP_H(i,:)*(1/MAPw)*(MAPw-i+1);
    end
elseif Sim == 7
    for i = 1:MAPw      % Prefered location about horizontal line
        PREFER_MAP_H(i,:) = PREFER_MAP_H(i,:)*(1/MAPw)*(MAPw-i+1);
...
% If caluculate another parameter, following variables.

%{
Pprob = zeros(nuh,Tact,trialN);     % Probabilities for actions.

Gh = zeros(nuh,Tact,trialN);        % Expected free energy.
Fh = zeros(nuh,Tact,trialN);        % Marginal free energy.

beta = 1.0;
E = ones(nuh,1);                    % Setting fixed-form prior distribution over policies(habits)
G_error = zeros(Tact,trialN);       % G error(namely Affective charge)
%}
...
for k = 1:trialN    % Number of trials.
    ...
    for i = 1:Tact  % Number of actual time steps.
        ...
        G_error(i,k) = EFE_Precision_Updating_robothuman(E,Fh(:,i,k),Gh(:,i,k),beta);
        %}
...
for k = 1:trialN
    ...
    %{
    Figure(k) = figure('Name','Result','Position',[500,0,1200,800]);
    ...
    p2 = plot(1.5:1:Tact+0.5,-G_error(:,k));

    xlabel('time step',FontSize=14);
    ylabel('AC',FontSize=14);
    title('AC during each time steps',FontSize=16,FontWeight='bold');
    xticks(1:1:Tact+1);
    xlim([1 Tact+1]);
    hline = refline(0,0);
    hline.Color = 'r';
    hline.LineStyle = '--';
    idxneg = find(-G_error(:,k) >= 0);
    p2.MarkerIndices = idxneg;
    p2.Marker = '*';
    p2.MarkerSize = 10;
    ...
    for j = 1:nuh
        bubblechart(1.5:1:Tact+0.5,n_policy(j,:),Pprob(j,:,k));hold on;
    end

    xlabel('time step',FontSize=14);
    ylabel('policy',FontSize=14);
    title('Probability over each policy',FontSize=16,FontWeight='bold');
    xticks(1:1:Tact+1);
    xlim([1 Tact+1]);
    yticks(1:nuh);
    yticklabels({'below','straight','above'});
    ax = gca;
...
% ------------------------------------------------------------------------
% If caluculate G error, use code available from supplemental code from
% "Tutorial" that caluculates the value from free energies.
% It can be refered as "EFE_Precision_Updating.m"
% ------------------------------------------------------------------------

%{
function G_error=EFE_Precision_Updating_robothuman(E,F,G,beta)

gamma_0 = 1/beta;            % Starting expected free energy precision value
...
for ni = 16 % number of variational updates (16)
